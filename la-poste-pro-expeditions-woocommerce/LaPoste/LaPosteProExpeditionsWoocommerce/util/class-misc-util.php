...
class Misc_Util {
	...
	public static function get_active_parcel_point_networks( $settings ) {
		if ( ! isset( $settings['laposteproexp_parcel_point_networks'] )
			|| null === $settings['laposteproexp_parcel_point_networks']
			|| ! is_array( $settings['laposteproexp_parcel_point_networks'] )
			|| empty( $settings['laposteproexp_parcel_point_networks'] ) ) {
			return array();
		}

		$networks = Configuration_Util::get_network_list();
		...
		return array_values( array_intersect( $networks_array, $settings['laposteproexp_parcel_point_networks'] ) );
	}
