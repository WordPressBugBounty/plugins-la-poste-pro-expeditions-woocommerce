...
namespace LaPoste\LaPosteProExpeditionsWoocommerce;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

use LaPoste\LaPosteProExpeditionsWoocommerce\Init\Environment_Check;
use LaPoste\LaPosteProExpeditionsWoocommerce\Init\Setup_Wizard;
use LaPoste\LaPosteProExpeditionsWoocommerce\Init\Api_Action;
use LaPoste\LaPosteProExpeditionsWoocommerce\Notice\Notice_Controller;
use LaPoste\LaPosteProExpeditionsWoocommerce\Shipping_Method\Parcel_Point\Checkout;
use LaPoste\LaPosteProExpeditionsWoocommerce\Shipping_Method\Parcel_Point\Render;
use LaPoste\LaPosteProExpeditionsWoocommerce\Shipping_Method\Settings_Override;
use LaPoste\LaPosteProExpeditionsWoocommerce\Subscription\Admin_Subscription_Page;
use LaPoste\LaPosteProExpeditionsWoocommerce\Subscription\Front_Subscription_Page;
...
class Plugin implements \ArrayAccess {
	...
	public function woocommerce_shipping_methods_filter( $methods ) {
		$methods['la_poste_pro_expeditions_woocommerce'] = 'LaPoste\LaPosteProExpeditionsWoocommerce\Shipping_Method\Shipping_Method';
		return $methods;
	}
	...
	public function register_shipping_pethod() {
		if ( $this->environment_has_no_errors() && $this->can_use_plugin() ) {
			...
			$controller = new \LaPoste\LaPosteProExpeditionsWoocommerce\Shipping_Method\Controller( $this );
			$controller->run();
		}
	}
	...
	public function register_parcel_point() {
		if ( $this->environment_has_no_errors() && $this->can_use_plugin() ) {
			...
			$controller = new \LaPoste\LaPosteProExpeditionsWoocommerce\Shipping_Method\Parcel_Point\Controller( $this );
			$controller->run();
	...
	public function plugins_before_woocommerce_init_action() {
		if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
			$plugin_name = 'la-poste-pro-expeditions-woocommerce/la-poste-pro-expeditions-woocommerce.php';
	...
	public function wpmu_new_blog_action( $blog_id, $user_id, $domain, $path, $site_id, $meta ) {
		if ( is_plugin_active_for_network( 'la-poste-pro-expeditions-woocommerce/la-poste-pro-expeditions-woocommerce.php' ) ) {
			switch_to_blog( $blog_id );
			Database_Util::create_tables();
			restore_current_blog();
		}
	...
	public function wpmu_drop_tables_action( $tables, $blog_id ) {
		global $wpdb;
		$tables[] = $wpdb->prefix . 'laposteproexp_pricing_items';
		return $tables;
	}
}
