...
class Admin_Subscription_Page {
	...
	public function add_parcelpoint_to_admin_subscription_page( $post_type, $post = null ) {
		...
		$title = sprintf( __( '%s - Shipment pickup point', 'la-poste-pro-expeditions-woocommerce' ), 'La Poste Pro Expéditions' );

		if ( function_exists( 'wcs_get_page_screen_id' ) ) {
			$subscription_screen_id = wcs_get_page_screen_id( 'shop_subscription' );
			add_meta_box( 'lpfr-eco-subscription-parcelpoint', $title, array( $this, 'subscription_edit_page_parcelpoint' ), $subscription_screen_id, 'side', 'default' );

		} elseif ( function_exists( 'wc_get_order_types' ) ) {
			foreach ( wc_get_order_types( 'order-meta-boxes' ) as $type ) {
				add_meta_box( 'lpfr-eco-subscription-parcelpoint', $title, array( $this, 'subscription_edit_page_parcelpoint' ), $type, 'side', 'default' );
			}
		} else {
			add_meta_box( 'lpfr-eco-subscription-parcelpoint', $title, array( $this, 'subscription_edit_page_parcelpoint' ), 'shop_subscription', 'side', 'default' );
		}
	}
