...
class Admin_Order_Page {
	...
	public function add_parcelpoint_to_admin_order_page_hpos( $order ) {
		...
		if ( null !== $this->parcelpoint ) {
			...
			$box_name = sprintf( __( '%s - Shipment pickup point', 'la-poste-pro-expeditions-woocommerce' ), 'La Poste Pro Expéditions' );
			$box_id   = 'lpfr-eco-order-parcelpoint';
	...
	public function add_parcelpoint_to_admin_order_page() {
		...
		if ( function_exists( 'wc_get_order_types' ) ) {
			foreach ( wc_get_order_types( 'order-meta-boxes' ) as $type ) {
				/* translators: 1) plugin name */
				add_meta_box( 'lpfr-eco-order-parcelpoint', sprintf( __( '%s - Shipment pickup point', 'la-poste-pro-expeditions-woocommerce' ), 'La Poste Pro Expéditions' ), array( $this, 'order_edit_page_parcelpoint' ), $type, 'side', 'default' );
			}
		} else {
			/* translators: 1) plugin name */
			add_meta_box( 'lpfr-eco-order-parcelpoint', sprintf( __( '%s - Shipment pickup point', 'la-poste-pro-expeditions-woocommerce' ), 'La Poste Pro Expéditions' ), array( $this, 'order_edit_page_parcelpoint' ), 'shop_order', 'side', 'default' );
		}
	}
	...
	public function add_tracking_to_admin_order_page_hpos( $order ) {
		...
		if ( null !== $this->tracking && property_exists( $this->tracking, 'shipmentsTracking' ) && ! empty( $this->tracking->shipmentsTracking ) ) {
			...
			$box_name = sprintf( __( '%s - Shipment tracking', 'la-poste-pro-expeditions-woocommerce' ), 'La Poste' );
			$box_id   = 'lpfr-eco-order-tracking';
	...
	public function add_tracking_to_admin_order_page() {
		...
		if ( function_exists( 'wc_get_order_types' ) ) {
			foreach ( wc_get_order_types( 'order-meta-boxes' ) as $type ) {
				/* translators: 1) company name */
				add_meta_box( 'lpfr-eco-order-tracking', sprintf( __( '%s - Shipment tracking', 'la-poste-pro-expeditions-woocommerce' ), 'La Poste' ), array( $this, 'order_edit_page_tracking' ), $type, 'normal', 'high' );
			}
		} else {
			/* translators: 1) company name */
			add_meta_box( 'lpfr-eco-order-tracking', sprintf( __( '%s - Shipment tracking', 'la-poste-pro-expeditions-woocommerce' ), 'La Poste' ), array( $this, 'order_edit_page_tracking' ), 'shop_order', 'normal', 'high' );
		}
	}
