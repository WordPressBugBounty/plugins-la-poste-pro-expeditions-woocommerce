...
class Controller {
	...
	public function run() {
		...
		add_action( 'wp_ajax_laposteproexp_get_map_url', array( $this, 'get_map_url_callback' ) );
		...
		add_action( 'wp_ajax_nopriv_laposteproexp_get_map_url', array( $this, 'get_map_url_callback' ) );
	...
	public function parcel_point_scripts() {
		if ( $this->is_checkout_or_cart() ) {
			...
			wp_enqueue_script( 'laposteproexp_maplibre_gl', $this->plugin_url . 'LaPoste/LaPosteProExpeditionsWoocommerce/assets/js/maplibre-gl.js', array( 'laposteproexp_polyfills' ), $this->plugin_version, false );
	...
	public function parcel_point_styles() {
		if ( $this->is_checkout_or_cart() ) {
			wp_enqueue_style( 'laposteproexp_maplibre_gl', $this->plugin_url . 'LaPoste/LaPosteProExpeditionsWoocommerce/assets/css/maplibre-gl.min.css', array(), $this->plugin_version );
